...
Spotify.prototype._openWebsocket = function (err, res) {
  ...
  var url = 'wss://lon2-linkap-a5.ap.spotify.com:443/';
...
Spotify.prototype._onmessagecommand = function (command, args) {
  if ('do_work' == command) {
    ...
    var js = args[0][1];
    try {
      vm.runInContext("window = {eval: eval}; " + js, this.createCleanContext());
    } catch (e) {
      this.emit('error', e);
    }
  } else {
...
/**
 * Creates a clean context for the request to run in.
 */
Spotify.prototype.createCleanContext = function (){
  this._context = vm.createContext();
  this._context.reply = this._reply.bind(this);
...
Spotify.prototype.sendPong = function (ping) {
  function ping_pong(paddle) {
    for (var ab = paddle.split(' '), a = {}, L = 0; L < ab.length; L++) {
      var wa = parseInt(ab[L]); a[4 * L + 1] = wa & 255; a[4 * L + 2] = wa >> 8 & 255;
      a[4 * L + 3] = wa >> 16 & 255; a[4 * L + 4] = wa >> 24 & 255
    } var W, M, Ka, b, x, ia, N, bb, xa, O, X, La, q, ya, d, ja, C, l, g, D, Ma, ka, r, e, la, Y, za,
...
Spotify.prototype.isTrackAvailable = function (track, country) {
  ...
  if (Array.isArray(track.restriction)) {
    for (var i = 0; i < track.restriction.length; i++) {
      ...
      var isAllowed = !restriction.hasOwnProperty('countriesAllowed') || restriction.countriesAllowed === null || has(allowed, country);
